$(document).ready(function() {
    ...
    $('#intraBankTransferForm').on('submit', function(event) {
        ...
        $.ajax({
            url: `http://localhost:8082/api/fundtransfer/intra`,
            type: 'POST',
            headers: {
                'Authorization': `Bearer ${authToken}`,
            ...
            success: function(data) {
                ...
                // show proper response message in rotating animated window
                $('#successModal').modal('show');

                $('#intraBankTransferForm')[0].reset();
            },
            error: function(xhr) {
